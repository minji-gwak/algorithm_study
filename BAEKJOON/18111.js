...
//       if (blockHeight > flatHeight) {
//         let removeCount = blockHeight - flatHeight; // 제거하는 블록 개수
//         currentHaveBlocksCount += removeCount; // 제거한만큼 인벤토리에 추가
//         takenTime = takenTime + 2 * removeCount; // 제거할 때는 개당 2초씩 소요
//       } else if (blockHeight < flatHeight) {
//         let addCount = flatHeight - blockHeight; // 추가하는 블록 개수
//         currentHaveBlocksCount -= addCount; // 추가한만큼 인벤토리에서 제거
//         takenTime = takenTime + addCount; // 추가할 때는 개당 1초씩 소요
//       }
//     });

//     // 정상적으로 평지를 만들 수 있는 경우라면, 인벤토리에 있는 블럭의 수는 음수일 수 없다.
//     if (currentHaveBlocksCount >= 0) {
//       if (minTime > takenTime) {
//         // 시간 소모가 더 짧은 경우를 발견했을 때
//         minTime = takenTime;
